...
###################################################################################
###################################################################################
#The following is a modified version of ConsensusClusterPlus, making use of the parallelDist and fastcluster packages to obtain speedup of clustering times
        ...
        1) && is.null(weightsFeature)))) {
            if (inherits(distance, "character")) {
                ...
                "function") {
                    ...
                    message("Boon, this is where it's working...")
                    main.dist.obj <- parDist(t(d), method = distance)
            ...
            if (clusterAlg != "km") {
                ...
                "function")) {
                    ...
                    if (distance == "pearson" | distance == "spearman") {
                        ...
                        message("It's working here now, Boon...")
                        this_dist <- parDist(t(sample_x$submat), method = distance)
...
ConsensusCluster <- function (d = NULL, maxK = 3, reps = 10, pItem = 0.8,
pFeature = 1, clusterAlg = "hc", title = "untitled_consensus_cluster",
...
corUse = "everything")
{
    ...
    if (is.null(ml) == TRUE) {
        ...
        if (inherits(d, "dist")) {
            ...
            if (is.null(distance) || (distance != attr(d,
            "method"))) {
        ...
        print("Calling ml function")
        ml <- ccRun(d = d, maxK = maxK, repCount = reps, diss = inherits(d, "dist"), pItem = pItem, pFeature = pFeature,innerLinkage = innerLinkage, clusterAlg = clusterAlg,weightsFeature = weightsFeature, weightsItem = weightsItem,distance = distance, verbose = verbose, corUse = corUse)
    }
    res = list()
    ...
    col = tmyPal, na.rm = TRUE, labRow = rownames(sc),
    labCol = F, main = "consensus matrix legend")
    print("beginning to merge subsamples...")
    for (tk in 2:maxK) {
        if (verbose) {
        ...
        c = fm
        colorList = setClusterColors(res[[tk - 1]][[3]],
        ct, thisPal, colorList)
        pc = c
        pc = pc[hc$order, ]
    ...
    print("Finished merging subsamples, creating CDF")
    CDF(ml)
