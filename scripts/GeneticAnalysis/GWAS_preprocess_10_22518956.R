...
for (i in 1:nrow(tissue_x_celltype)) {
  ...
  for (k in 1:3) {
    ...
    snp_id <- '10_22518956_A_G_b37'
    df.geno <- fread(paste0('/athena/elementolab/scratch/anm2868/GTEx/GTEx_infil/output/GeneticAnalysis/Genotype_Subset/gtex_all.',snp_id,'.raw'),data.table=F,stringsAsFactors = F)
    colnames(df.geno)[ncol(df.geno)] <- 'SNP'
    df.geno$IID <- as.character(lapply(strsplit(df.geno$IID,'-'),function(x) {paste(x[1:2],collapse = '-')}))
    df.sub.pca <- merge(df.sub.pca,df.geno[,c('IID','SNP')],by.x='ID',by.y='IID')
    ...
    if (tis %in% c('Ovary','Uterus','Vagina','Testis','Prostate')) {
      ...
      mod <- lm(df.sub.pca[,cell]~SNP+as.numeric(as.factor(AGE)) + as.factor(DTHHRDY) + as.factor(SEX) + as.numeric(SMATSSCR) + as.factor(SMCENTER) + PC1 + PC2 + PC3,data=df.sub.pca,na.action=na.exclude)
